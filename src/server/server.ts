...
export const startServer = ({
  ...
  // Static client.
  authenticated.get('/', (req, res) => sendPage(req, res, LandingPage, {}));
  // Video app.
    ...
    const videosByDir = videos.reduce((folders, v, i) => {
      ...
      folders.get(folderName)!.push({
        ...
        preview: v.preview && {
          ...
          height: v.preview.height,
          width: v.preview.width,
          montageFrames: v.preview.montageFrames.map(f => ({
            time: f.time,
            URL: `/video/${i}/montage/${f.time}`,
          })),
        },
      });
      return folders;
  ...
  authenticated.get('/video/:videoId/thumbnail', (req, res) => {
    ...
    res.sendFile(preview.thumbnailPath);
  });
  authenticated.get('/video/:videoId/montage/:time', async (req, res) => {
    const video: Video = videos[req.params.videoId];
    if (!video) {
      return res.status(404).end();
    }
    ...
    const time = Number.parseInt(req.params.time, 10);
    const frame = preview.montageFrames.find(f => f.time == time);
    if (!frame) {
      return res.status(404).end();
    }
